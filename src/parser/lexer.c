...
void lex_ungetr(Lexer* l) {
    l->r = l->r0;
    l->line = l->line0;
    l->col = l->col0;
}
void lex_ungetr2(Lexer* l) {
    lex_ungetr(l);
    l->r0--;
    l->col0--;
}
void lex_number(Lexer* l, i32 c) {
    ...
    l->nlsemi = 1;
}int lex_escape(Lexer* l, char quote) {
    int n;
    u32 base, max;
...
#define LEX_CASE1(c1, t1, c2, t2) case c1:\
    ...
    else { lex_ungetr(l); l->token.tok = t1; }\
    break;
...
#define LEX_CASE3(c1, t1, c21, t21, c22, t22, c3, t3) case c1:\
    ...
    if(c == c21) { l->token.tok = t21; }\
        ...
        else { lex_ungetr(l); l->token.tok = t22; }\
    } else { lex_ungetr(l); l->token.tok = t1; }\
    break;
